...
class AsyncChunkInferenceWorker:
    """Single-threaded asynchronous model worker with task/request filtering."""

    def __init__(self, model, wait_timeout: float = 0.03, queue_size: int = 2):
        self.model = model
        self.wait_timeout = float(wait_timeout)
        qsize = max(1, int(queue_size))
        self._request_queue: queue.Queue[tuple[int, int, dict[str, Any]] | None] = queue.Queue(
            maxsize=qsize
        )
        self._result_queue: queue.Queue[tuple[int, int, Any, float]] = queue.Queue(
            maxsize=qsize
        )
    ...
    def get_latest_matching_request(self, request_id: int, task_epoch: int):
        ...
        while True:
            try:
                ...
                break
            epoch, rid, actions, latency = item
            if epoch == task_epoch and rid == request_id:
                hit = (rid, actions, latency)
            elif epoch >= task_epoch:
                stash.append(item)
        for item in stash:
            try:
        ...
        try:
            while True:
                ...
                if epoch == task_epoch and rid == request_id:
                    for saved in stash:
                        try:
                            self._result_queue.put_nowait(saved)
                        except queue.Full:
                            pass
                    return rid, actions, latency
                stash.append(item)
        finally:
            for saved in stash:
                try:
            ...
            try:
                ...
                try:
                    self._result_queue.put_nowait(
                        (task_epoch, request_id, actions, latency)
                    )
                except queue.Full:
                    pass
            except Exception:
                ...
                try:
                    self._result_queue.put_nowait(
                        (task_epoch, request_id, None, latency)
                    )
                except queue.Full:
                    pass
