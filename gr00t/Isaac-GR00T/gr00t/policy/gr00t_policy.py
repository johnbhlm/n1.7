...
class Gr00tPolicy(BasePolicy):
    ...
    def _get_action(
        ...
        use_rtc = bool(options and options.get("use_rtc", False))

        if use_rtc and self._rtc_prev_normalized_action is not None:
            rtc_action_horizon = int(options["action_horizon"])
            rtc_overlap_steps = int(options["rtc_overlap_steps"])
            rtc_frozen_steps = int(options["rtc_frozen_steps"])
            rtc_ramp_rate = float(options.get("rtc_ramp_rate", 10.0))

            if not 0 <= rtc_frozen_steps <= rtc_overlap_steps <= rtc_action_horizon:
                raise ValueError(
                    "RTC requires: "
                    "0 <= rtc_frozen_steps <= rtc_overlap_steps <= action_horizon"
                )

            current_batch_size = collated_inputs["inputs"]["state"].shape[0]
            previous_batch_size = self._rtc_prev_normalized_action.shape[0]

            if current_batch_size != previous_batch_size:
                # Batch size changed; discard stale RTC state.
                self._rtc_prev_normalized_action = None
            else:
                # The model's prepare_input() will move this tensor to GPU.
                collated_inputs["inputs"]["action"] = (
                    self._rtc_prev_normalized_action.to(dtype=torch.bfloat16)
                )

                rtc_model_options = {
                    # This is the valid predicted chunk length,
                    # not the client execution horizon.
        ...
        with torch.inference_mode():
            model_pred = self.model.get_action(
                **collated_inputs,
                options=rtc_model_options,
            )

        normalized_action = model_pred["action_pred"].float()

        if use_rtc:
            # Keep the full padded/model-space chunk.
            self._rtc_prev_normalized_action = (
